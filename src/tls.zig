...
pub const Stream = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !*Stream {
        var ret = try allocator.create(Stream);
    ...
    pub fn wrap(self: *Stream, stream: std.net.Stream) !void {
        ...
        const certificate = @embedFile("cacert.pem");
        err = mbedtls.mbedtls_x509_crt_parse(&self.cert, certificate, certificate.len+1);
        if (err != 0) {
            return error.CertificateParseFailed;
        }
        mbedtls.mbedtls_ssl_conf_ca_chain(&self.conf, &self.cert, null);
    ...
    pub fn read(self: *Stream, buffer: []u8) ReadError!usize {
        ...
        if (ret < 0 and ret != mbedtls.MBEDTLS_ERR_SSL_WANT_READ and ret != mbedtls.MBEDTLS_ERR_SSL_WANT_WRITE
            and ret != mbedtls.MBEDTLS_ERR_SSL_ASYNC_IN_PROGRESS and ret != mbedtls.MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS) {
